  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/dashboard') {
        const db = await getDb(); const users = db.collection('group_users'); const groupStats = db.collection('group_stats'); const groupSettings = db.collection('group_settings');
        ...
        const settingsDocs = await groupSettings.find({}).toArray();
        const regionMap = new Map(settingsDocs.map(x=>[String(x.groupId),x.region||'🌐 International']));
        const allowedGroupIds = selectedRegion ? settingsDocs.filter(x=>(x.region||'🌐 International')===selectedRegion).map(x=>String(x.groupId)) : null;
        const regionalMatch = selectedRegion ? { ...match, groupId: { $in: allowedGroupIds } } : match;
        const [userRows, groupRows, userCount, messageAgg, groupCount] = await Promise.all([
          users.aggregate([{ $match: regionalMatch },{ $group:{_id:'$userId',displayName:{$last:'$displayName'},userName:{$last:'$userName'},value:{$sum:field},groupCount:{$sum:1}}},{ $sort:{value:-1}},{ $limit:100}]).toArray(),
          users.aggregate([{ $match: regionalMatch },{ $group:{_id:'$groupId',groupName:{$last:'$groupName'},value:{$sum:field},activeUsers:{$sum:1}}},{ $sort:{value:-1}},{ $limit:100}]).toArray(),
        ...
        const genderDocs = userRows.length ? await db.collection('user_genders').aggregate([{ $match:{userId:{$in:userRows.map(x=>String(x._id))}}},{ $sort:{updatedAt:-1}},{ $group:{_id:'$userId',gender:{$first:'$gender'}}}]).toArray() : []; const genderMap=new Map(genderDocs.map(x=>[String(x._id),x.gender])); return json(res,{mode,regions:[...new Set(settingsDocs.map(x=>x.region||'🌐 International'))].sort(),stats:{users:userCount[0]?.n||0,messages:messageAgg[0]?.n||0,groups:groupCount[0]?.n||0},users:userRows.map(x=>({...x,userId:x._id,gender:genderMap.get(String(x._id))||null})),groups:groupRows.map(x=>({...x,groupId:x._id,region:regionMap.get(String(x._id))||'🌐 International',memberCount:members.get(String(x._id)) ?? null}))});
      }
      const groupApiMatch = url.pathname.match(/^\/api\/group\/([^/]+)$/);
      if (req.method === 'GET' && groupApiMatch) {
        const groupId = decodeURIComponent(groupApiMatch[1]); const db = await getDb(); const users = db.collection('group_users'); const groupStats = db.collection('group_stats'); const groupSettings = db.collection('group_settings');
        ...
        const [settingsDoc, groupGenderDocs] = await Promise.all([groupSettings.findOne({groupId:String(groupId)}), db.collection('user_genders').find({groupId:String(groupId),userId:{$in:topUsers.map(x=>String(x.userId))}}).toArray()]); const groupGenderMap=new Map(groupGenderDocs.map(x=>[String(x.userId),x.gender])); topUsers.forEach(x=>x.gender=groupGenderMap.get(String(x.userId))||null); const rank = rankRows.findIndex(x=>String(x._id)===String(groupId))+1; const activeUsers = await users.countDocuments({ groupId: String(groupId) }); const t=totals[0]||{};
        return json(res,{mode,group:{groupId,groupName:statsDoc?.groupName||base.groupName||'Unknown Group',groupLink:statsDoc?.groupLink||base.groupLink||null,region:settingsDoc?.region||'🌐 International',memberCount:statsDoc?.memberCount ?? null,activeUsers,totalMessages:t.total||0,rank:rank||'-'},activity:{today:t.today||0,weekly:t.weekly||0,total:t.total||0},users:topUsers});
      }
      ...
      if (req.method === 'GET' && userApiMatch) {
        ...
        const base=baseRows[0]; const genderDoc=await db.collection('user_genders').find({userId:String(userId)}).sort({updatedAt:-1}).limit(1).next(); if (!base) return json(res,{error:'User not found'},404); const t=totals[0]||{};
        ...
        return json(res,{mode,user:{userId,displayName:base.displayName||null,userName:base.userName||null,gender:genderDoc?.gender||null,totalMessages:t.total||0,groupCount:groups.length,rank:rank||'-',periodValue},activity:{today:t.today||0,weekly:t.weekly||0,total:t.total||0},groups});
      }
